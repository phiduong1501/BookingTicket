                ...
                BookingFactory.UpdateStatusPayment(/*$scope.ListTempID $scope.model.ListID*/$scope.model.ListID, 1, $scope.model.RouteID, function (response) {
                    if (response.Success) {
                        ...
                        for (var i = 0; i < lstSeat.length; i++) {
                            ...
                            //nhảy ra trang in vé bằng html

                            var WinPrint = window.open("/Ticket/PrinterTicket?lstCarDateGoDetailID=" + $scope.model.ListID, "MsgWindow", "width=800,height=600,rigth=100");
                            WinPrint.focus();
